...
function stockBadge(p: Producto) {
  ...
  if (p.estado.includes("SOLICITAR"))                  return { label: "Poco stock", color: YELL };
  if (p.stock < 5)                                     return { label: `Últimas ${p.stock}`, color: YELL };
      ...
      <div style={{ background: "#12141e", borderBottom: `1px solid ${BORD}`, padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {phone && (
          <a
            ...
            style={{ background: GREEN, color: "#fff", padding: "8px 18px", borderRadius: 20, fontSize: 13, fontWeight: 600, textDecoration: "none", display: "flex", alignItems: "center", gap: 6 }}
          >
            WhatsApp
          </a>
        )}
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: TEAL }}>
                {p.precio > 0 ? `${p.precio.toLocaleString("es-ES")} CUP` : <span style={{ fontSize: 13, color: MUTED }}>Consultar precio</span>}
                {p.udm && p.precio > 0 && <span style={{ fontSize: 11, color: MUTED, fontWeight: 400 }}> / {p.udm}</span>}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                {p.stock > 0 && (
                  <span style={{ fontSize: 11, color: MUTED }}>{p.stock} {p.udm}</span>
                )}
              </div>
              ...
              {phone && disponible && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  style={{ marginTop: "auto", background: PRP, color: "#fff", textAlign: "center", padding: "9px 0", borderRadius: 10, fontSize: 13, fontWeight: 600, textDecoration: "none" }}
                >
                  Pedir por WhatsApp
                </a>
              )}
